...
for line in graph_string.splitlines():
    if line:
        ...
        graph[start] = tuple(end[1:-1].split(', '))

# move_idx, pos, res = 0, 'AAA', 0
# while pos != 'ZZZ':
#     move = moves[move_idx]
#     pos = graph[pos][0] if move == 'L' else graph[pos][1]
#     move_idx = (move_idx + 1) % len(moves)
#     res += 1
# print(res)

gen = [node for node in graph if node[-1] == 'A']
complete = []
steps, move_idx = 0, 0
while gen:
    new_gen = []
    for node in gen:
        if node[-1] == 'Z': complete.append(steps)
        else: new_gen.append(graph[node][0] if moves[move_idx] == 'L' else graph[node][1])
    move_idx, gen, steps = (move_idx + 1) % len(moves), new_gen, steps + 1
